...
export class ProductsService {
  ...
  async getProductLocationData(
    ...
    if (!product) {
      const memberWithProduct =
        await this.memberService.findByEmailNotThrowError(productId);
    ...
    const orderOrigin = isCreating
      ? 'XX'
      : this.getLocationCode(product.location ?? 'Unknown');
    let orderDestination = this.getLocationCode(product.location ?? 'Unknown');
    const origin = isCreating ? '' : product.location ?? 'Unknown';
    let destination = product.location ?? 'Unknown';

    // 📌 Si la ubicación es un Employee, usamos el código del país y el nombre completo
    if (product.location === 'Employee') {
      const member = await this.memberService.findByEmailNotThrowError(
        product.assignedEmail!,
      );
      if (member && member.country) {
        orderDestination = this.getCountryCode(member.country); // 📌 Código del país para Order ID
        destination = `${member.firstName} ${member.lastName}`; // 📌 Nombre completo en BD
      }
    }

    return { product, origin, destination, orderOrigin, orderDestination };
  }

  private async isAddressComplete(
  ...
  private async determineProductStatus(
    product: Partial<Product>,
    tenantName: string,
  ): Promise<Status> {
    ...
    const isComplete = await this.isAddressComplete(product, tenantName);

    return isComplete ? 'In Transit' : 'In Transit - Missing Data';
  }
