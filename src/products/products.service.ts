...
export class ProductsService {
  ...
  async bulkCreate(
    ...
    try {
      ...
      // 🔎 **Paso 2: Verificamos que los productos creados existen en BD**
      console.log(
        `🔍 Verificando productos creados antes de procesar envíos...`,
      );
      const productIds = createdProducts.map((p) => p._id!.toString());
      const allVerifiedProducts =
        await this.shipmentsService.findProductsInAllCollections(productIds);

      if (allVerifiedProducts.length !== createdProducts.length) {
        console.warn(
          `⚠️ Algunos productos no fueron encontrados tras la creación, omitiendo shipment.`,
        );
      } else {
        console.log(
          `📦 Procesando órdenes de envío para productos con fp_shipment = true...`,
        );
        console.log(
          `🔍 Productos encontrados antes de shipment:`,
          allVerifiedProducts.map((p) => (p._id ? p._id.toString() : '')),
        );

        try {
          // 🔹 **Paso 3: Llamamos a `processBulkShipments` pero SIN transacción**
          await this.shipmentsService.processBulkShipments(
            allVerifiedProducts,
            'bulkCreate',
          );
        } catch (shipmentError) {
          console.error(
            `🚨 Error al procesar órdenes de envío: ${shipmentError.message}`,
          );
        }
      }
