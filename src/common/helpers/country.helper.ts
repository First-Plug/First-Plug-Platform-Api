...
export class CountryHelper {
  ...
  /**
   * Obtiene todos los códigos válidos (solo ISO)
   */
  static getAllValidCodes(): string[] {
    return [...this.VALID_ISO_COUNTRY_CODES];
  }
  ...
  /**
   * Verifica si es una ubicación especial (Our office, FP warehouse)
   * Estas ubicaciones NO se migran, se mantienen como texto
   * @param location - Ubicación a verificar
   * @returns true si es una ubicación especial
   */
  static isSpecialLocation(location: string): boolean {
    if (!location || typeof location !== 'string') return false;
    const normalized = location.trim();
    return normalized === 'Our office' || normalized === 'FP warehouse';
  }
