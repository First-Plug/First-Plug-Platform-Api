...
#### **Gestión Automática de Oficina Default (NUEVO)**

- ✅ **Primera oficina** creada se convierte automáticamente en default
- ✅ **Eliminación de default** asigna automáticamente nueva oficina default
- ✅ **Solo una oficina default** por tenant en todo momento
- ✅ **Validación automática** de integridad de oficinas default

```typescript
// ✅ CORRECTO: Gestión automática en OfficesService
async ensureDefaultOffice(tenantName: string): Promise<void> {
  // Lógica automática para mantener siempre una oficina default
}
```
...
| `POST /assignments/offboard`      | `OffboardMemberDto`     | ✅ **Offboarding con oficinas específicas** |

#### **Movimientos Entre Oficinas (NUEVO)**

- ✅ **Oficina A → Oficina B** dentro de `location = "Our office"`
- ✅ **Detección automática** de cambios de `officeId` aunque `location` sea igual
- ✅ **Creación de shipments** para movimientos entre oficinas
- ✅ **Preservación de datos** de oficina origen en shipment

```typescript
// ✅ CORRECTO: Detección de cambio de oficina
...
#### **Patrón de Asignación con Persistencia (CORE)**
...
3. **❌ Romper la coexistencia**
   ...
   - Siempre verificar: "¿Este método maneja `fpWarehouse`?"
...
## 📦 Inyección de Modelos (PATRÓN CORE - CRÍTICO)

### **✅ CORRECTO: Inyectar el modelo en el constructor**
...
export class QuotesService {
  ...
  async findAll(userEmail: string): Promise<Quote[]> {
    // ✅ Usar this.quoteRepository directamente
...
### **❌ INCORRECTO: Obtener el modelo de la conexión en cada método**

```typescript
// ❌ NO HACER ESTO - Patrón antiguo
...
1. **Consistencia**: El modelo inyectado se resuelve automáticamente en el contexto del tenant actual
2. **Middleware**: El middleware de tenant ya ha resuelto la conexión correcta
3. **Limpieza**: Menos código repetitivo en cada método
4. **Performance**: Evita obtener la conexión múltiples veces
...
#### **2. Importar en el módulo del servicio**

```typescript
@Module({
  providers: [
    QuotesService,
    tenantModels.quoteModel, // ✅ Registrar el modelo
  ],
})
...
### **Cuándo usar `TenantConnectionService`**

Solo usar `getTenantConnection()` cuando necesites:

1. **Transacciones**: `connection.startSession()`
2. **Operaciones atómicas**: Múltiples modelos en una transacción
3. **Acceso directo a colecciones**: `connection.db.collection()`
4. **Modelos dinámicos**: Crear modelos sobre la marcha

```typescript
// ✅ Correcto: Usar para transacciones
...
- [ ] ¿Necesita acceso a datos? → Inyectar modelo en constructor
- [ ] ¿El modelo está registrado en `tenant-models-provider.ts`?
- [ ] ¿El módulo importa el modelo?
- [ ] ¿Se usa `TenantConnectionService` solo para transacciones?
- [ ] ¿Se removieron parámetros `tenantName` no utilizados?
...
**Síntomas**: Productos movidos entre oficinas no generaban shipments aunque `fp_shipment: true`
**Solución**: Detectar cambios de oficina además de cambios de location

```typescript
// ✅ CORRECTO: Detección de cambios de oficina
...
### **10. Offboarding con oficinas específicas**

**Causa**: El offboarding no soportaba asignación a oficinas específicas, solo oficina genérica
**Síntomas**: Productos offbordeados siempre iban a oficina default, no a oficina seleccionada
**Solución**: Agregar soporte para `officeId` en datos de offboarding

```typescript
// ✅ CORRECTO: Soporte de officeId en offboarding
data: Array<{
  product: any;
  ...
  fp_shipment?: boolean;
  officeId?: string; // ← NUEVO: Oficina específica
}>;
```

### **11. Oficinas duplicadas por nombre sin considerar país (RESUELTO)**

**Causa**: Validación de unicidad de oficinas solo verificaba `name`, no la combinación `name + country`
**Síntomas**: No se podía crear "Oficina Brasil" en BR si ya existía "Oficina Brasil" en AR
**Solución**: Actualizar validación en 3 métodos de `OfficesService`:

```typescript
// ✅ CORRECTO: Validación de nombre + país
const existingOfficeWithNameAndCountry = await OfficeModel.findOne({
  name: { $regex: new RegExp(`^${createOfficeDto.name}$`, 'i') },
  country: createOfficeDto.country, // ← AGREGAR: Validar país también
  isDeleted: false,
});
...
- ✅ `createOffice()` - Valida `name + country`
- ✅ `updateDefaultOffice()` - Valida `name + country`
- ✅ `updateOffice()` - Valida `name + country`

### **12. Productos en bulkCreate sin sincronización a colección global (RESUELTO)**

**Causa**: `ProductsService.bulkCreate()` creaba productos pero no sincronizaba `fpWarehouse` y `memberData` a la colección global
**Síntomas**:

- Productos con `location: "FP warehouse"` tenían `fpWarehouse: null` en global collection
- Productos con `location: "Employee"` tenían `memberData: null` en global collection

**Solución**: Poblar `productWarehouseMap` después de generar IDs:

```typescript
// ✅ CORRECTO: Poblar productWarehouseMap para sincronización
...
**Resultado**: Ahora la sincronización en líneas 693-753 de `bulkCreate()` incluye correctamente:

- ✅ `memberData` para productos asignados a members
- ✅ `fpWarehouse` para productos en FP warehouse
- ✅ `office` para productos en oficinas

---

## � CSV Upload - Productos y Members
...
```typescript
// ✅ CORRECTO: Validación condicional por location
if (assignedEmail) {
  ...
  // Warehouse asignado automáticamente
} else if (location === 'Our office') {
  // country REQUERIDO
  // officeName REQUERIDO
  // Oficina creada o reutilizada automáticamente
}
```

#### **Conversión de Códigos de País**

- ✅ **Acepta nombres completos**: "Brazil", "Argentina", "Barbados"
- ✅ **Acepta códigos ISO**: "BR", "AR", "BB"
- ✅ **Convierte automáticamente**: "Brazil" → "BR"
- ✅ **Usa mapping**: `countryCodes` de `src/shipments/helpers/countryCodes.ts`
- ✅ **Búsqueda case-insensitive**: "brazil" = "Brazil" = "BR"

**Implementación**: `CSVOfficeCoordinatorService.getCountryCode()`

#### **Creación de Oficinas desde CSV**

- ✅ **Validación**: `name + country` (no solo `name`)
- ✅ **Reutilización**: Si existe oficina con mismo `name + country`, se reutiliza
- ✅ **Creación**: Si no existe, se crea automáticamente
- ✅ **Sincronización**: Oficina se sincroniza a colección global

**Implementación**: `CSVOfficeCoordinatorService.handleOfficeAssignmentForCSV()`

#### **Sincronización Global desde CSV**

- ✅ **Todos los productos** se sincronizan a `firstPlug.global_products`
- ✅ **Incluye `fpWarehouse`** para productos en "FP warehouse"
- ✅ **Incluye `memberData`** para productos asignados a members
- ✅ **Incluye `office`** para productos en "Our office"

**Implementación**: `ProductsService.bulkCreate()` líneas 693-753
...
| ----------- | ------ | --------- | -------------------------- |
| `firstName` | string | ✅        | Nombre del member          |
| `lastName`  | string | ✅        | Apellido del member        |
| `email`     | string | ✅        | Email único por tenant     |
| `position`  | string | ✅        | Puesto/rol                 |
| `dni`       | string | ❌        | **NUEVO**: DNI/CI/Passport |
| `phone`     | string | ❌        | Teléfono                   |
| `city`      | string | ❌        | Ciudad                     |
| `country`   | string | ❌        | País (código ISO o nombre) |

#### **Campo DNI (NUEVO)**

- ✅ **Opcional**: No requerido en CSV
- ✅ **Acepta letras y números**: "12345678A", "V-87654321", "123456789"
- ✅ **Único por tenant**: Validación de unicidad
- ✅ **Trim automático**: Elimina espacios al inicio/final
- ✅ **Almacenamiento**: Se guarda en `members.dni`

**Implementación**: `MembersService.create()` y `bulkCreate()`

---

## 📦 CSV de Productos - Nuevas Columnas (ACTUALIZADO)

### **Columnas Soportadas en CSV**

| Columna            | Tipo    | Requerido     | Comportamiento                                              |
| ------------------ | ------- | ------------- | ----------------------------------------------------------- |
| `name`             | string  | ❌ Opcional\* | \*Requerido si category es "Merchandising" o "Other"        |
| `category`         | enum    | ✅ Sí         | Computer, Monitor, Audio, Peripherals, Merchandising, Other |
| `serialNumber`     | string  | ❌ Opcional   | Único por tenant, se convierte a minúsculas                 |
| `productCondition` | enum    | ❌ Opcional   | Optimal, Defective, Unusable - **Default: Optimal**         |
| `recoverable`      | boolean | ❌ Opcional   | **Default: Usa config del tenant**                          |
| `additionalInfo`   | string  | ❌ Opcional   | Información adicional del producto                          |
| `price`            | object  | ❌ Opcional   | `{ amount: number, currencyCode: string }`                  |
| `location`         | enum    | ✅ Sí         | Employee, Our office, FP warehouse                          |
| `assignedEmail`    | string  | ❌ Opcional   | Email del member (solo si location=Employee)                |
| `country`          | string  | ❌ Opcional\* | \*Requerido si location es "Our office" o "FP warehouse"    |
| `officeName`       | string  | ❌ Opcional\* | \*Requerido si location es "Our office"                     |
...
```typescript
// Siempre sobrescribía a 'Optimal'
...
**Ubicación:** `src/products/products.service.ts` línea 540-543

#### **2. Validación de `name` Extendida**
...
**Ubicación:** `src/products/validations/create-product.zod.ts` línea 569-579

#### **3. Schema Zod Actualizado**

```typescript
productCondition: z.enum(CONDITION).optional(), // 🔧 Opcional en CSV
```

**Ubicación:** `src/products/validations/create-product.zod.ts` línea 462

### **Comportamiento de Defaults**

| Campo              | Si está vacío en CSV | Comportamiento                                    |
| ------------------ | -------------------- | ------------------------------------------------- |
| `productCondition` | ✅ Vacío             | Se asigna `'Optimal'`                             |
| `recoverable`      | ✅ Vacío             | Se usa `isRecoverableConfig[category]` del tenant |
| `additionalInfo`   | ✅ Vacío             | Se deja vacío (opcional)                          |
| `price`            | ✅ Vacío             | Se deja vacío (opcional)                          |

### **Validaciones Aplicadas**

✅ **Price + Currency:** Ambos deben estar presentes o ambos vacíos
✅ **Name:** Requerido para "Merchandising" y "Other"
✅ **Country:** Requerido para "Our office" y "FP warehouse"
✅ **OfficeName:** Requerido para "Our office"
✅ **AssignedEmail:** Solo permitido si location es "Employee"

---

## 🚢 Offboarding - Tracking de Shipments (NUEVO)

### **Método `offboardMember()` - Retorno Actualizado**
...
- ✅ **Rastreo automático**: Captura el ID del último shipment creado durante el offboarding
- ✅ **Consolidación soportada**: Si múltiples productos se consolidan en un shipment, retorna el ID del último
- ✅ **Sin shipments**: Si no se crean shipments, `lastShipmentCreated` es `undefined`
- ✅ **Múltiples destinos**: Cuando hay diferentes destinos (member, office, warehouse), retorna el ID del último shipment procesado

#### **Implementación Técnica**

```typescript
// En offboardMember() - línea 2728
let lastShipmentCreated: ObjectId | null = null; // 🚢 Rastrear último shipment

// En el loop - línea 2798-2800
if (result.shipment?._id) {
  lastShipmentCreated = result.shipment._id;
}

// En el return - línea 2872-2875
return {
...
```
Producto 1 → Shipment A (nuevo) → lastShipmentCreated = A._id
Producto 2 → Shipment A (consolidado) → lastShipmentCreated = A._id
Producto 3 → Shipment B (nuevo) → lastShipmentCreated = B._id
Producto 4 → Shipment B (consolidado) → lastShipmentCreated = B._id
...
- **Retorno**: Automáticamente incluye `lastShipmentCreated`
...
- [ ] **📦 MODELOS**: ¿Necesita inyectar modelos en el servicio? (Ver sección "Inyección de Modelos")
...
- [ ] **📦 MODELOS**: ¿Inyecta modelos en constructor? (NO obtener de conexión en métodos)
...
## 📎 Attachments en IT Support Services (Release 2)
...
- **`AttachmentsService`**: CRUD de attachments en Quote.services[].attachments
...
- **`AttachmentsCoordinatorService`**: Coordina Storage + Attachments
  - `uploadAndPersist(quoteId, file)` - validar → subir → persistir
  - `deleteAttachment(quoteId, publicId)` - remover de Quote + borrar de Cloudinary

**Servicios Externos:**

- **`StorageService`**: Agnóstico (Cloudinary MVP, S3 futuro)
...
```typescript
// ✅ CORRECTO: Controller → Coordinador
AttachmentsController → AttachmentsCoordinatorService

// ✅ CORRECTO: Coordinador → Servicios raíz + Storage
AttachmentsCoordinatorService → StorageService + AttachmentsService

// ✅ CORRECTO: Servicio raíz → Modelo
AttachmentsService → @Inject('QUOTE_MODEL')

// ❌ PROHIBIDO: Servicio raíz → Servicio raíz
AttachmentsService ❌→ StorageService

// ❌ PROHIBIDO: Servicio raíz → Servicio raíz
QuotesService ❌→ StorageService
```

#### **Flujo Opción A: Upload Temporal (IMPLEMENTADO)**

```
1. POST /quotes/:quoteId/services/it-support/attachments
   ├─ AttachmentsCoordinatorService.uploadAndPersist()
   ├─ Validar: MIME (jpeg, png, webp), tamaño (5MB), cantidad (4 máx)
   ├─ Subir a Cloudinary via StorageService
   ├─ Persistir en Quote via AttachmentsService
   └─ Retorna: { publicId, secureUrl, createdAt, expiresAt, ... }

2. GET /quotes/:quoteId/services/it-support/attachments
   └─ Front ve preview de imágenes

3. Si user hace SUBMIT
   └─ Imagen ya está persistida en Quote ✅

4. Si user NO hace SUBMIT
   └─ Cron diario borra attachments con expiresAt < now ✅
```

#### **Manejo de Imágenes No Submiteadas**

**DECISIÓN FINAL: Limpieza al Cambiar Status (NO Cron)**

- ✅ **Imagen se persiste inmediatamente** en Quote (cuando sube)
- ✅ **Mientras status = `requested`**: imágenes NECESARIAS
- ✅ **Cuando status = `cancelled`**: limpiar attachments
  - Borrar imágenes de Cloudinary
  - Vaciar array de attachments en Quote
  - Quote permanece como registro histórico
  - Inmediato, sin delay
- ✅ **Cuando status = `accepted`** (futuro): decidir si guardar o borrar

**Razón**: Las imágenes solo sirven mientras la quote está en estado `requested`. Una vez cancelada, no tienen valor. Quote siempre permanece como historial.
...
export class AttachmentSchema {
  @Prop({ type: String, enum: ['cloudinary', 's3'], required: true })
  provider: 'cloudinary' | 's3';

  @Prop({ type: String, required: true, index: true })
  publicId: string; // ID en provider (necesario para borrar)

  @Prop({ type: String, required: true })
  secureUrl: string; // URL pública (para Slack, UI)

  @Prop({ type: String, enum: ['image/jpeg', 'image/png', 'image/webp'] })
  mimeType: string;

  @Prop({ type: Number, required: true, min: 0, max: 5242880 })
  bytes: number; // 5MB máx
  ...
  expiresAt: Date; // Para búsquedas de cleanup
}
```

#### **Integración en ITSupportServiceSchema**
...
export class ITSupportServiceSchema extends BaseServiceSchema {
  ...
  attachments?: AttachmentSchema[]; // Array de adjuntos
}
```
...
| ------ | ------------------------------------------------------------ | -------------------- |
| POST   | `/quotes/:quoteId/services/it-support/attachments`           | Subir imagen         |
| GET    | `/quotes/:quoteId/services/it-support/attachments`           | Obtener para preview |
| DELETE | `/quotes/:quoteId/services/it-support/attachments/:publicId` | Borrar imagen        |

#### **Validaciones**

- ✅ MIME: `image/jpeg`, `image/png`, `image/webp`
- ✅ Tamaño: máx 5MB
- ✅ Cantidad: máx 4 por servicio IT Support
- ✅ Expiración: 30 días desde creación

#### **Plan de Salida (Migración a S3)**

Cuando sea necesario cambiar a S3:

1. Crear `src/storage/providers/s3.provider.ts` implementando `StorageProvider`
2. Cambiar en `storage.module.ts`:
   ```typescript
   {
   ...
   ```
3. **Listo.** El resto del código no cambia (AttachmentsService, AttachmentsCoordinatorService, etc.)

---

## 🚨 Reglas SAGRADAS (NUNCA ROMPER)

### **1. Inyección de Modelos (CRÍTICO - PATRÓN CORE)**

- ✅ **SIEMPRE** inyectar modelos en el constructor del servicio
- ✅ **Registrar** en `tenant-models-provider.ts`
- ✅ **Importar** en el módulo del servicio
- ✅ **Usar** `this.quoteRepository` directamente en métodos
- ❌ **PROHIBIDO**: Obtener modelo de conexión en cada método
- ❌ **PROHIBIDO**: `const QuoteModel = connection.model('Quote', QuoteSchema)`
- ✅ **CORRECTO**: `@Inject('QUOTE_MODEL') private quoteRepository: Model<QuoteDocument>`

**Razón**: El modelo inyectado se resuelve automáticamente en el contexto del tenant actual gracias al middleware.

### **2. Arquitectura de Servicios (CRÍTICO)**
...
### **3. Attachments en IT Support (CRÍTICO - Release 2)**

#### **Servicios Correctos**

- ✅ **`AttachmentsService`** (Raíz): CRUD de attachments en Quote

  - Solo inyecta: `@Inject('QUOTE_MODEL')`
  - Métodos: `addAttachment()`, `removeAttachment()`, `getAttachments()`

- ✅ **`AttachmentsCoordinatorService`** (Transversal): Coordina Storage + Attachments
  - Inyecta: `StorageService` + `AttachmentsService`
  - Métodos: `uploadAndPersist()`, `deleteAttachment()`

#### **Dependencias Prohibidas**

- ❌ **PROHIBIDO**: `AttachmentsService` → `StorageService` (directo)
- ❌ **PROHIBIDO**: `QuotesService` → `StorageService` (directo)
- ❌ **PROHIBIDO**: `QuotesService` → `AttachmentsService` (directo)

#### **Flujo Correcto**

```
Controller → AttachmentsCoordinatorService → StorageService + AttachmentsService
```

#### **Manejo de Imágenes No Submiteadas**

**DECISIÓN FINAL: Limpieza al Cambiar Status**

- ✅ Imagen se persiste **inmediatamente** en Quote (cuando sube)
- ✅ Mientras status = `requested`: imágenes NECESARIAS
- ✅ Cuando status = `cancelled`: limpiar attachments
  - `AttachmentsCoordinatorService.cleanupAttachmentsOnCancel(quoteId)`
  - Borrar imágenes de Cloudinary
  - Vaciar array de attachments en Quote
  - Quote permanece como registro histórico
  - Inmediato, sin delay
- ✅ Integrado en `QuotesCoordinatorService.cancelQuoteWithCoordination()`

**Razón**: Las imágenes solo sirven mientras la quote está en estado `requested`. Una vez cancelada, no tienen valor. Quote siempre permanece como historial. Limpieza inmediata, sin cron.
